...
int main(int argc, char *argv[])
{
    ...
    {
        char domain[200], path[200], garbage[200], end[200];      //buffer for domain,path,some unnecessary values and the file type like .gif , .pdf etc
        ...
        snprintf(data_to_send, sizeof(data_to_send), "GET /%s HTTP/1.0\r\nHost: %s\r\n\r\n", path, domain); //data_to_send is now contains the http request of the proper format
        //HTTP 1.0 is non-persistent by default unless we specify connection=keep-alive in the http header
...
//used HTTP 1.0 in the http request header so that the server can know that we want a non-persistent connection as it is non-persistent by default
//unless otherwise requested as connection=keep-alive in the header(i.e for persistent connection)